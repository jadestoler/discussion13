...
def create_employee_table(cur, conn):
    cur.execute("CREATE TABLE IF NOT EXISTS employees (employee_id PRIMARY KEY, first_name TEXT, last_name TEXT, job_id INT, hire_date INT, salary INT)")
    conn.commit()
...
def add_employee(filename, cur, conn):
    ...
    cur.execute("INSERT OR IGNORE INTO employees (employee_id, first_name, last_name, job_id, hire_date, salary) VALUES (?,?,?,?,?,?), (employee_id, employee_first_name, employee_last_name, employee_job_id, employee_hire_date, employee_salary)")
    conn.commit()
